...
import config
from concurrent.futures import TimeoutError as FuturesTimeoutError, ProcessPoolExecutor, as_completed
...
PARSE_TIMEOUT_SEC        = config.PARSE_TIMEOUT_SEC

TABLE_CROP_PADDING           = config.TABLE_CROP_PADDING
TABLE_TRANSFORMER_THRESHOLD  = config.TABLE_TRANSFORMER_THRESHOLD
TABLE_DEDUP_IOU_THRESHOLD    = config.TABLE_DEDUP_IOU_THRESHOLD
CELL_SHRINK_MARGIN_PTS       = config.CELL_SHRINK_MARGIN_PTS
MAX_NEAREST_CELL_DIST_SQ     = config.MAX_NEAREST_CELL_DIST_SQ
COMPLEX_TEXT_THRESHOLD       = config.COMPLEX_TEXT_THRESHOLD
COMPLEX_TOTAL_THRESHOLD      = config.COMPLEX_TOTAL_THRESHOLD

_BOX_XMIN, _BOX_YMIN, _BOX_XMAX, _BOX_YMAX = 0, 1, 2, 3

_FIGURE_GARBAGE_RE = config.FIGURE_GARBAGE_RE
LAYOUT_NMS_IOU_THRESHOLD = config.LAYOUT_NMS_IOU_THRESHOLD
TEXT_CLIP_PADDING_X      = config.TEXT_CLIP_PADDING_X
TEXT_CLIP_PADDING_Y      = config.TEXT_CLIP_PADDING_Y
...
def _nms_layout_boxes(yolo_boxes, iou_threshold: float = LAYOUT_NMS_IOU_THRESHOLD) -> list[int]:
    """Non-Maximum Suppression for YOLO layout boxes to remove overlapping detections.
    Returns list of kept box indices (sorted by original order)."""
    n = len(yolo_boxes)
    if n == 0:
        return []

    # Extract data: (original_index, x1, y1, x2, y2, confidence)
    items = []
    for idx in range(n):
        b = yolo_boxes[idx]
        bx1, by1, bx2, by2 = b.xyxy[0].tolist()
        conf = b.conf[0].item()
        items.append((idx, bx1, by1, bx2, by2, conf))

    # Sort by confidence descending — higher confidence survives
    items.sort(key=lambda x: x[5], reverse=True)

    kept: list[int] = []
    suppressed: set[int] = set()

    for i, (idx, x1, y1, x2, y2, conf) in enumerate(items):
        ...
        for j in range(i + 1, len(items)):
            oidx, ox1, oy1, ox2, oy2, oconf = items[j]
            if oidx in suppressed:
                continue
            ...
            if iou > iou_threshold:
                suppressed.add(oidx)
                logger.debug(
                    "NMS: suppressed box %d (conf=%.2f) overlaps box %d (conf=%.2f) iou=%.2f",
                    oidx, oconf, idx, conf, iou,
                )
...
class DocumentParser:
    ...
    def _process_page(
        ...
        # ── NMS: filter overlapping layout boxes ──────────────────────
        kept_indices = set(_nms_layout_boxes(yolo_boxes))
        if len(kept_indices) < len(yolo_boxes):
            logger.info(
                "Page %d: NMS filtered %d → %d boxes",
                page_no, len(yolo_boxes), len(kept_indices),
            )
        ...
        for idx in sorted_indices:
            ...
            # Pad YOLO boxes to prevent edge character clipping
            rect = fitz.Rect(
                max(0, bx1 * scale_x - TEXT_CLIP_PADDING_X),
                max(0, by1 * scale_y - TEXT_CLIP_PADDING_Y),
                min(page.rect.width,  bx2 * scale_x + TEXT_CLIP_PADDING_X),
                min(page.rect.height, by2 * scale_y + TEXT_CLIP_PADDING_Y),
            )
            ...
            if label in text_labels:
                ...
                # Near-duplicate containment check (length-aware)
                normalized_lower = re.sub(r'\s+', ' ', raw_text.lower()).strip()
                is_near_dup = False
                if len(normalized_lower) >= 100:
                    for existing_norm in seen_normalized:
                        shorter, longer = sorted([normalized_lower, existing_norm], key=len)
                        if len(shorter) >= 100 and shorter in longer and (len(shorter) / len(longer)) > 0.95:
                            is_near_dup = True
                            logger.debug("Near-duplicate text suppressed on page %d", page_no)
                            break
                if is_near_dup:
                    continue
